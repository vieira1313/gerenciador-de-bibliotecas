...
class livroControler{
 async createLivro (req, res)  {
    ...
    const informacoes = true;
      
        disponivel: true
    
    await knex("livro").insert({title, autor, paginas, categoria, disponivel})
...
async updateLivro(req, res) {
    ...
    await knex("livro").where({id:livro_id}).update({title, autor, paginas, categoria, disponivel})
